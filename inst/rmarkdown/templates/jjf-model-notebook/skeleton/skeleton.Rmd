...
features <- firstSummaryDF %>% 
  select(
         ...
         -ends_with("requests"),
         -ends_with("letters"),
         -starts_with("days_to_first"),
         -starts_with("days_to_last"),
         -final_account_position,
         -is_paid,
         -actual_placement, 
         -created_date, 
         -note_date_time, 
         -placement_date,
         -facility_id                    # handled by facility name
  )
```
...
```{r}
recipeDF <- recipe(final_outcome_is_paid ~ ., data = features) %>% 
  step_other(all_predictors(), -all_numeric(), threshold=0.025, other="OTHER") %>% 
...
dtrain <- xgb.DMatrix(
  data = data.matrix(select(train_data, -final_outcome_is_paid)),
  label = data.matrix(select(train_data, final_outcome_is_paid)))

dtest <- xgb.DMatrix(
  data = data.matrix(select(test_data, -final_outcome_is_paid)),
  label = data.matrix(select(test_data, final_outcome_is_paid)))
```
...
f_train_new <- train_data %>% 
  mutate(target = as.factor(final_outcome_is_paid)) %>% 
  select(-final_outcome_is_paid)
...
sampleDF <- cleanDF %>% inner_join(accountsDF, by="id")
```
...
patientsDB <- cleanDF %>% 
  select(id, 
         ...
         patient_age, 
         patient_zip_code,
         tgt = final_outcome_is_paid) %>% 
  group_by(id) %>% 
  summarize_all(max, na.rm=T) %>% 
  mutate(tgt=as.factor(tgt))   


events_touch <- cleanDF %>% 
  select(dt = note_date_time,
         id,
  ...
  pivot_longer(cols = c(ends_with("touches"), 
               ends_with("requests"), 
               ends_with("letters")), 
               names_to="event_data",
               values_to="data" ) %>% 
  filter(data != 0) %>% 
  filter(!is.na(event_data)) %>% 
  distinct() %>% 
  extract(event_data, c("event_desc", "event"), "([[[:alnum:]]+_]*)_([[:alnum:]]+)$")
   

eventsDB <- patientsDB %>% 
  left_join(events_touch, by="id") %>% 
  filter(!is.na(event)) %>% 
  distinct() 
```

```{r, fig.height=8, fig.width=8}
touchEvents <- eventsDB %>% filter(event == "touches")
requestEvents <- eventsDB %>% filter(event == "requests")
letterEvents <- eventsDB %>% filter(event == "letters")

eventsDB %>% ggplot() +
  geom_point(data=eventsDB, aes(x=dt, y=id, color=tgt),size=2)
...
```{r}
features %>% 
  tabyl(created_by_user, final_outcome_is_paid) %>% 
